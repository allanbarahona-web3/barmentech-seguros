    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        doc
          .fontSize(20)
          .fillColor('#2563eb')
          .text(settings.companyName || 'Barmentech Seguros', {
            align: 'center',
          })
          .moveDown(0.5);
        ...
        if (phoneNumbers && phoneNumbers.length > 0) {
          doc.text(`${phoneNumbers[0].country}: ${phoneNumbers[0].phone}`, {
            align: 'center',
          });
        }
        ...
        if (data.travelDateFrom && data.travelDateTo) {
          const dateFrom = new Date(data.travelDateFrom).toLocaleDateString(
            'es-ES',
          );
          const dateTo = new Date(data.travelDateTo).toLocaleDateString(
            'es-ES',
          );
        ...
        doc
          .fontSize(14)
          .fillColor('#2563eb')
          .text('Cobertura General', { underline: true })
          .moveDown(0.5);

        doc.fontSize(10).fillColor('#111827');
        doc.text(`Monto Máximo Global: ${data.globalMaxAmount}`, {
          bold: true,
        });
        if (data.validityTerritory)
          doc.text(`Validez Territorial: ${data.validityTerritory}`);
        if (data.consecutiveDays)
          doc.text(`Días Consecutivos: ${data.consecutiveDays}`);
        doc.moveDown(1);
        ...
        doc
          .fontSize(14)
          .fillColor('#2563eb')
          .text('Coberturas Incluidas', { underline: true })
          .moveDown(0.5);
        ...
        doc.text('Descripción', colDescription + 5, tableTop + 5, {
          width: 320,
        });
        ...
        data.coverages.forEach((coverage, index) => {
          ...
          doc.text(coverage.description, colDescription + 5, currentY + 5, {
            width: 320,
          });
          ...
          if (coverage.notes) {
            ...
            doc.text(`Nota: ${coverage.notes}`, colCode + 5, currentY + 3, {
              width: 490,
            });
            currentY += 15;
          }
        ...
        doc
          ...
          .text(
            `Fecha de emisión: ${new Date(data.createdAt).toLocaleDateString('es-ES')}`,
            { align: 'center' },
          )
          .moveDown(0.3);
        ...
        doc
          .text(
            'Este documento es una cotización y no constituye un contrato de seguro.',
            { align: 'center' },
          )
          .text(
            'Para activar la cobertura, debe proceder con el pago y emisión formal de la póliza.',
            { align: 'center' },
          );
