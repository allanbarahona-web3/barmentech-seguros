...
export class AuthService {
  ...
  async register(registerDto: RegisterDto, ip: string = 'unknown') {
    ...
    if (registerDto.honeypot && registerDto.honeypot.trim() !== '') {
      this.securityLogger.logSuspiciousActivity(
        'Honeypot triggered on register',
        {
    ...
    const { honeypot: _honeypot, ...userDataClean } = registerDto;
  ...
  async validateActivationToken(
    ...
    try {
      ...
      if (user.isActivated) {
        return {
          valid: false,
          message: 'Esta cuenta ya fue activada. Inicia sesión.',
        };
      }
      ...
      if (user.tokenExpiry && new Date() > user.tokenExpiry) {
        return {
          valid: false,
          message: 'El token ha expirado. Contacta al administrador.',
        };
      }
      ...
      this.securityLogger.logSuspiciousActivity(
        'Error validating activation token',
  ...
  async activateAccount(
    ...
    if (user.tokenExpiry && new Date() > user.tokenExpiry) {
      this.securityLogger.log(
        `Expired activation token for user: ${user.email}`,
        'AuthService',
      );
      throw new BadRequestException(
        'Activation token has expired. Please contact administrator.',
      );
    }
    ...
    if (user.isActivated) {
      throw new BadRequestException(
        'Account is already activated. Please login.',
      );
    }
    ...
    this.securityLogger.log(
      `Account activated successfully: ${user.email}`,
      'AuthService',
    );
